...
def test_positive_correlation() -> None:
    ...
    for i in range(10):
        t.record(component="momentum_30d", score=float(i), forward_return=float(i) * 0.5, ts_ms=i * 1000)
    snap = t.compute_ic("momentum_30d")
...
def test_negative_correlation() -> None:
    t = ICTracker()
    for i in range(10):
        t.record(component="funding_yield", score=float(i), forward_return=float(-i), ts_ms=i * 1000)
